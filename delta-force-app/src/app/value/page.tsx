  ...
  const items = useMemo(() => {
    ...
    for (const [tk, cat] of Object.entries(data.attachments)) {
      for (const acc of cat.items) {
        ...
        if (s.recoil) stats.push({ key: "recoil", label: "后坐力", val: Math.abs(s.recoil) });
        if (s.controlSpeed) stats.push({ key: "controlSpeed", label: "操控速度", val: Math.abs(s.controlSpeed) });
        if (s.controlStable) stats.push({ key: "controlStable", label: "据枪稳定", val: Math.abs(s.controlStable) });
        if (s.extraBullet) stats.push({ key: "extraBullet", label: "额外弹容", val: Math.abs(s.extraBullet) });
